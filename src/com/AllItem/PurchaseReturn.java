...
public class PurchaseReturn extends JPanel{
	...
	BufferedImage image;
	public PurchaseReturn(SessionBeam sessionbeam) {
...
/*				ResultSet rs1=db.sta.executeQuery("select ledgerId from accfledger where ledgerTitle='"+cmbSupplierName.txtMrNo.getText().trim().toString()+"' && depId='701'");
				...
				String sql="update tbinvoice set cash=cash+'"+cashAmount+"',card=card+'"+cardAmount+"',paid=paid+'"+paid+"', date=NOW(),entryTime=NOW(),createBy='"+sessionbeam.getUserId()+"' where invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' && type=2";
				System.out.println(sql);
				db.sta.executeUpdate(sql);
			...
			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==KeyEvent.VK_ENTER){
					try {
						...
						String sql="select * from tbpharmacystore where invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' && type=2";
						System.out.println(sql);
						...
						String sql1="select tbinvoice.invoiceNo,"
								...
								+ "tbinvoice.paid,"
								+ "tbinvoice.date"
								+ " from tbinvoice where tbinvoice.invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' && tbinvoice.type=2";
	...
	private boolean UpInvoicedoplicateName(){
		try {
			ResultSet rs=db.sta.executeQuery("select productName,buyPrice from tbpharmacystore where type='2' and invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'");
			while(rs.next()){
	...
	private boolean InvoicedoplicateName(){
		try {
			ResultSet rs=db.sta.executeQuery("select productName,buyPrice from tbpharmacystore where type='2' and invoiceNo IS NULL");
			while(rs.next()){
				if(cmbProductName.txtMrNo.getText().toString().equalsIgnoreCase(rs.getString("productName"))){
					return true;
				}
		...
		try {
			if(findId==1){
				if(! UpInvoicedoplicateName()){
					...
					JOptionPane.showMessageDialog(null, "Purchase Return Successfully Accept");
					UpdateInvoieValue();
					GrossAmount();
					txtDiscountPercent.setText("0");
					txtManualDiscount.setText("0");
					...
					qty=Integer.parseInt(txtQty.getText().trim().toString());
					price=Double.parseDouble(txtPrice.getText().trim().toString());
					ResultSet rs=db.sta.executeQuery("select qty from tbpharmacystore where productName='"+cmbProductName.txtMrNo.getText().toString()+"' && type='2' && invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'");
					while(rs.next()){
						totalqty=totalqty+Integer.parseInt(rs.getString("qty"));
					}
					totalPrice=totalqty*price;
					String query="update tbpharmacystore set qty='"+totalqty+"',buyPrice='"+price+"',totalPrice='"+df.format(totalPrice)+"' where productName='"+cmbProductName.txtMrNo.getText().toString()+"' && type='2' && invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'";
					System.out.println(query);
					db.sta.executeUpdate(query);
					JOptionPane.showMessageDialog(null, "Product Retrun Successfully Accept");
					UpdateInvoieValue();
					GrossAmount();
					ProductTxtClear();
				}
				...
				if(!InvoicedoplicateName()){
					...
					ResultSet rs=db.sta.executeQuery("select qty from tbpharmacystore where productName='"+cmbProductName.txtMrNo.getText().toString()+"' && type='2' && invoiceNo IS NULL");
					while(rs.next()){
						totalqty=totalqty+Integer.parseInt(rs.getString("qty"));
					}
					totalPrice=totalqty*price;
					String query="update tbpharmacystore set qty='"+totalqty+"',buyPrice='"+price+"',totalPrice='"+df.format(totalPrice)+"' where productName='"+cmbProductName.txtMrNo.getText().toString()+"' && type='2' && invoiceNo IS NULL";
					System.out.println(query);
					db.sta.executeUpdate(query);
					JOptionPane.showMessageDialog(null, "Product Return Successfully Accept");
					tableValue();
					GrossAmount();
					ProductTxtClear();
				}
						...
						if(paid>Double.parseDouble(txtNetAmount.getText().trim().toString())){
							...
							if(findId==1){
								...
								if(confrim==JOptionPane.YES_OPTION){
									...
									JOptionPane.showMessageDialog(null, "Purchase Return Transection Complete");
									ClearAfterConfrim();
									findId=0;
								}
							}
	...
	public void autoinvoiceId(){
		try {
			String sql="select (ifnull(max(invoiceNo),0)+1)as invoiceNo from tbinvoice where type=2 ";
			ResultSet rs=db.sta.executeQuery(sql);
			while(rs.next())
			{
	...
	public void tableValue(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select * from tbpharmacystore where invoiceNo IS NULL and type=2");
			while(rs.next()){
	...
	private void UpdateInvoieValue(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select * from tbpharmacystore where (invoiceNo='"+txtInvoiceNo.getText().toString()+"' or invoiceNo IS NULL) and type=2");
			while(rs.next()){
				model.addRow(new Object[]{rs.getString("autoId"),rs.getString("productName"),rs.getString("qty"),rs.getString("buyPrice"),rs.getString("totalPrice"),new ImageIcon("icon/delete.png")});
			}
			loaddRow();
