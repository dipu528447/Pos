...
public class SalesStatement extends JPanel{
	...
	String col[]={"Invoice No","Customer","Gross Amount","Net Amount","Cash", "Card","Due","Sale Time","PRINT"};
	Object row[][]={};
	...
	String type="";
	BufferedImage image;
	public SalesStatement(SessionBeam sessionbeam) {
	...
	private void submitBtnEvent() {
		try {
			refreshTable();
				String sql="select tbinvoice.invoiceNo,tbinvoice.customer,tbinvoice.amount,tbinvoice.netAmount,tbinvoice.cash,tbinvoice.card,tbinvoice.netAmount-(tbinvoice.cash+tbinvoice.card) as due,tbinvoice.entryTime from tbinvoice where tbinvoice.date between '"+new SimpleDateFormat("yyyy-MM-dd").format(txtStartDate.getDate())+"' and '"+new SimpleDateFormat("yyyy-MM-dd").format(txtEndDate.getDate())+"' && type=3";
				System.out.println(sql);
				ResultSet rs=db.sta.executeQuery("select tbinvoice.invoiceNo,tbinvoice.customer,tbinvoice.amount,tbinvoice.netAmount,tbinvoice.cash,tbinvoice.card,tbinvoice.netAmount-(tbinvoice.cash+tbinvoice.card) as due,tbinvoice.entryTime from tbinvoice where tbinvoice.date between '"+new SimpleDateFormat("yyyy-MM-dd").format(txtStartDate.getDate())+"' and '"+new SimpleDateFormat("yyyy-MM-dd").format(txtEndDate.getDate())+"' && type=3");
				while(rs.next()){
					model.addRow(new Object[]{rs.getString("invoiceNo"),rs.getString("customer"),rs.getString("amount"),rs.getString("netAmount"),rs.getString("cash"),rs.getString("card"),rs.getString("due"),rs.getString("entryTime"),new ImageIcon("icon/cprint.png")});
				}
